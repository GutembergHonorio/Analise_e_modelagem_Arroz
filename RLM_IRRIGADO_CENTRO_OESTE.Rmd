...
Tabelabruta<- read.csv("Merge_1980_2018.csv", header = TRUE, sep = ";", stringsAsFactors = FALSE) # tabela com os dados originais
...
modelo<-lm(FLO_M~., Temporario[c(-115,-70),])
summary(modelo)


```
Removendo outro genótipo não significativo
...
car::ols_test_breusch_pagan(modelo, rhs = TRUE, multiple = TRUE, p.adj = 'sidak')
```
```{r}
# Testes de hipóteses de interesse
...
m.C2 <- rbind(cbind(0,1,0,0,0),cbind(0,0,1,0,0))

m.C1 <- cbind(0,1,0,0,0)
testeF.CB(fit.model,m.C1)
...
variaveis = c('Tmax_Maxima','Tmax_fld_m',
              'Tmax_R','Tmax_ACC_R',
              'Tmax_Minima','Tmax_Media',
              'Tmin_Maxima','Tmin_ACC_V',
              'Tmin_R','Tmin_ACC_R',
              'longitude','Chuva_ACC_V')
...
# Renomeando colunas
colnames(predictMediana) = variaveis
colnames(predictModa) = variaveis
...
for(variavel in variaveis){
        ...
        # Calculando mediana
        calcModa = lapply(Temporario[,-indexVar],moda)
        calcModa = do.call('cbind',calcModa)
        calcModa = as.data.frame(calcModa)
        
        # Rodando predict
        tempData = cbind(tempData,calcModa)
        predictModa = cbind(predictModa,predict(fit.model,tempData))
}
