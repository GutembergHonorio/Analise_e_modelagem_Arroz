...
Nordeste$longitude = as.numeric(Nordeste$longitude)
```
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + longitude + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)
...
SELECAO_GEN <- c("ALIANCA","BRS JABURU",    "CICA 8",   "EMPASC 101",    "EPAGRI 109",   "MOXOTO",   "PR268"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS A704",  "BRS FORMOSO"   ,"EPAGRI 108"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS GUARA",     "IAC1298",   "IAC1299",    "PR267",    "SCS 114 ANDOSAN",   "SCS 121 CL"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRSMG RUBELITA"  , "SCS 116 SATORU",   "SCSBRS 113 TIO TAKA"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("IAC1307"  , "OUROMINAS"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS CATIANA"  ,"BRS ALVORADA"
)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + longitude + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    s(Chuva_ACC_R) + Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + longitude + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
     Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```

```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + s(latitude) + longitude + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
     Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + longitude + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + s(longitude) + Altitude + s(Tmin_Maxima) + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Rad_ACC , family = gaussian, data = Temporario)
...
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + longitude + Altitude + Tmin_Maxima + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + longitude + Altitude + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude + s(longitude) + Altitude  + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + latitude +  Altitude  + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + s(latitude) +Altitude  + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + Altitude  + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
    Chuva_ACC_R + Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO + s(Altitude)  + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC +  Rad_ACC , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO   + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
   Rad_ACC , family = gaussian, data = Temporario)
...
```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO   + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC + 
   s(Rad_ACC) , family = gaussian, data = Temporario)

summary(modelo_GAM_Nordeste)
```

```{r}
modelo_GAM_Nordeste<- gam(FLO_M ~ GENOTIPO   + Tmin_V + Tmin_ACC_V + s(Tmin_R) + Tmin_ACC_R + Chuva_ACC , family = gaussian, data = Temporario)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("ALIANCA","BRS A704",    "CICA 8",   "EMPASC 101",    "EPAGRI 109",   "MOXOTO",   "EPAGRI 108"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS JABURU"  ,"PR268"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS FORMOSO"  ,  "BRS GUARA"  ,  "BRSMG RUBELITA",      "SCS 114 ANDOSAN" ,    "SCS 121 CL"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("SCS 116 SATORU"  ,  "SCSBRS 113 TIO TAKA"
)
...
```{r}
Genotipos  <- Temporario$GENOTIPO
SELECAO_GEN <- c("BRS ALVORADA","BRS CATIANA","IAC1299","PR267"
)
