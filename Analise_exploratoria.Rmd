...
graf_ano_flo = ggplot(teste)+
  geom_smooth(aes(y=FLO_E , x = ano), se = FALSE,method = 'loess', span = 0.9)+
    labs(x = "Ano", y = "Florescimento (DAE)", title = "(A)")+
...
graf_ano_genotipo = ggplot(Ano_GENOTIPO)+
  geom_smooth(aes(y=FLO_M , x = ano), se = FALSE, span = 0.99)+
    labs(x = "Ano", y = "", title = "(B)")+
  scale_x_continuous(breaks=c(seq(1995,2018,5)))+
  scale_y_continuous(breaks=c(seq(80,95,2)))+
  theme_classic(base_size= 17)
```
```{r}
an_geno(graf_ano_flo | graf_ano_genotipo)
...
d<- data.frame("Regioes" = names(macrorregiao_Ensaios),"quant" = macrorregiao_Ensaios, "Legenda" = "Nº de ensaios realizados")
e<- data.frame("Regioes" = names(macrorregiao_Genotipo),"quant" = macrorregiao_Genotipo, "Legenda" = "Nº de genótipos testados")
aa<- bind_rows(d,e)
...
graf_ens_reg<-
  ...
  labs(y = "Macrorregiões", x= "", fill = NULL, title = "(A)")+
...
d<- data.frame("Estados" = names(Estados_Ensaios),"quant" = Estados_Ensaios, "Legenda" = "Nº de ensaios realizados")
e<- data.frame("Estados" = names(Estados_Genotipo),"quant" = Estados_Genotipo, "Legenda" = "Nº de genótipos testados")
aa<- bind_rows(d,e)
...
graf_ens_est<-
  ...
  geom_col(data= aa, aes(y=Estados , x = quant, fill = Legenda ) ,position = "dodge")+
    theme_classic(base_size = 15)+
  labs(y = "Estados", x= "", fill = NULL, title = "(B)")+
  scale_fill_viridis_d()+
...
```{r}
(graf_ens_reg | graf_ens_est)+plot_layout(guides = 'collect') & theme(legend.position='bottom', text = element_text(size=20))
```
...
aa <- data.frame("Ano" = names(Ano_ensaio), "Nº" = Ano_ensaio, "Legenda" = "Nº de ensaios no ano")

bb <- data.frame("Ano" = names(Ano_GEN), "Nº" = Ano_GEN, "Legenda" = "Nº de genótipos testados no ano")

cc <- data.frame("Ano" = names(Ano_GENOTIPO), "Nº" = Ano_GENOTIPO, "Legenda" = "Nº de genótipos lançados no ano")
...
```{r}
a2<- data.frame("Temperatura" = ad_variaveis$Tmax_Maxima, "REGIAO" = ad_variaveis$REGIAO, "Tipo" = "Máxima máxima")
b2<- data.frame("Temperatura" = ad_variaveis$T_Media, "REGIAO" = ad_variaveis$REGIAO, "Tipo" = "Média")
c2<- data.frame("Temperatura" = ad_variaveis$Tmin_Minima, "REGIAO" = ad_variaveis$REGIAO, "Tipo" = "Mínima mínima")
...
geom_boxplot(
  ...
  theme_classic()+
        theme(legend.position="none")+
       labs(y = "ºC",x= "", )+
  facet_wrap(~Tipo, strip.position = "left")
...
geom_boxplot(
  ...
  theme_classic()+
        theme(legend.position="none")+
       labs(y = "Umidade relativa do ar (%)",x= "")
```
```{r}
...
geom_boxplot(
  ...
  theme_classic()+
        theme(legend.position="none")+
       labs(y = "Precipitação acumulada (mm)",x= "")
```
```{r}
...
geom_boxplot(
  ...
  theme_classic()+
        theme(legend.position="none")+
       labs(y = "Radiação solar acumulada (MJ/m²)",x= "")
...
```{r}
a3<- data.frame("graus" = ad_variaveis$Graus_dia_ciclo, "REGIAO" = ad_variaveis$REGIAO, "Tipo" = "Graus_dia_ciclo")
b3<- data.frame("graus" = ad_variaveis$Graus_dia_FLO, "REGIAO" = ad_variaveis$REGIAO, "Tipo" = "Graus_dia_FLO")
temp<- data.frame()
temp<- bind_rows(a3,b3)

g<-  ggplot(temp)+
  geom_boxplot(aes(y=graus, color = Tipo))+
      labs(y = "Graus-dia (ºC)", color = NULL)+
...
geom_boxplot(
  ...
  theme_classic()+
        theme(legend.position="bottom")+
       labs(y = "Graus-dia (ºC)",x= "", color = NULL, fill = NULL)
